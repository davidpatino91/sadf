...
const CREDITOS_EJEMPLO = [
  {
    id: nuevoId(),
    banco: "Bancolombia",
    numeroObligacion: "4500-112233",
    descripcion: "Crédito capital de trabajo cosecha 2026",
    valorDesembolsado: 3200000000,
    fechaDesembolso: "2026-02-15",
    fechaVencimiento: "2031-02-15",
    plazoMeses: 60,
    spread: 3.25,
    periodicidadIntereses: "Mensual",
    periodicidadCapital: "Mensual",
    mesesGracia: 0,
    tipoAmortizacion: "capital_constante",
    estado: "Activo",
    observaciones: "Garantía FNG 60%.",
  },
  {
    id: nuevoId(),
    banco: "Finagro",
    numeroObligacion: "FG-2025-0987",
    descripcion: "Redescuento Finagro - renovación cultivo de palma",
    valorDesembolsado: 5800000000,
    fechaDesembolso: "2025-11-01",
    fechaVencimiento: "2033-11-01",
    plazoMeses: 96,
    spread: 2.10,
    periodicidadIntereses: "Trimestral",
    periodicidadCapital: "Trimestral",
    mesesGracia: 24,
    tipoAmortizacion: "capital_constante_gracia",
    estado: "En gracia",
    observaciones: "24 meses de gracia a capital.",
  },
  {
    id: nuevoId(),
    banco: "Davivienda",
    numeroObligacion: "DV-778-2024",
    descripcion: "Compra de maquinaria de extracción",
    valorDesembolsado: 950000000,
...
const IBR_EJEMPLO = [
  { id: nuevoId(), fecha: "2025-10-01", valorEA: 10.12 },
  { id: nuevoId(), fecha: "2025-11-01", valorEA: 9.99 },
  { id: nuevoId(), fecha: "2025-12-01", valorEA: 9.91 },
  { id: nuevoId(), fecha: "2026-01-01", valorEA: 9.87 },
  { id: nuevoId(), fecha: "2026-02-01", valorEA: 9.80 },
  { id: nuevoId(), fecha: "2026-03-01", valorEA: 9.74 },
  { id: nuevoId(), fecha: "2026-04-01", valorEA: 9.68 },
  { id: nuevoId(), fecha: "2026-05-01", valorEA: 9.61 },
  { id: nuevoId(), fecha: "2026-06-01", valorEA: 9.55 },
].map((t) => ({ ...t, fuente: "Manual" }));

let _idToast = 0;

/* ----------------------------------------------------------------------------
   4.4 — Persistencia, configuración inicial y manejo centralizado de errores
   ----------------------------------------------------------------------------
   Persistencia: usa el almacenamiento de artifacts (window.storage), que
   sobrevive a cerrar el navegador o reiniciar el equipo — a diferencia de
   localStorage/sessionStorage, que esta plataforma no soporta. Estas
   funciones son la única puerta de entrada/salida a ese almacenamiento;
   si en el futuro se reemplaza por una base de datos empresarial, solo hay
   que reescribir cargarStorage/guardarStorage — el Motor Financiero, que
   nunca toca el almacenamiento directamente, no se ve afectado.
   -------------------------------------------------------------------------- */
const CLAVES_STORAGE = {
  creditos: "sadf:creditos",
  historicoIBR: "sadf:historicoIBR",
  configuracion: "sadf:configuracion",
  auditoria: "sadf:auditoria",
};

async function cargarStorage(clave) {
  try {
    if (typeof window === "undefined" || !window.storage) return null;
    const r = await window.storage.get(clave, false);
    return r ? JSON.parse(r.value) : null;
  } catch {
    return null; // clave inexistente todavía, o almacenamiento no disponible
  }
}
async function guardarStorage(clave, valor) {
  try {
    if (typeof window === "undefined" || !window.storage) return false;
    await window.storage.set(clave, JSON.stringify(valor), false);
    return true;
  } catch (e) {
    registrarErrorSistema(e, `Guardar ${clave} en almacenamiento persistente`);
    return false;
  }
}
...
function AppProvider({ children }) {
  const [creditos, setCreditos] = useState(CREDITOS_EJEMPLO);
  const [historicoIBR, setHistoricoIBR] = useState(IBR_EJEMPLO);
  ...
  useEffect(() => {
    ...
    (async () => {
      const [c, i, cfg, aud] = await Promise.all([
        cargarStorage(CLAVES_STORAGE.creditos),
        cargarStorage(CLAVES_STORAGE.historicoIBR),
        cargarStorage(CLAVES_STORAGE.configuracion),
        cargarStorage(CLAVES_STORAGE.auditoria),
      ]);
      if (!vigente) return;
      setCreditos(c || CREDITOS_EJEMPLO);
      setHistoricoIBR(i || IBR_EJEMPLO);
  ...
  // ---- Guardado automático (solo después de terminar la carga inicial, para
  //      no sobrescribir datos reales con los datos de ejemplo) -------------
  useEffect(() => { if (!cargandoInicial) guardarStorage(CLAVES_STORAGE.creditos, creditos); }, [creditos, cargandoInicial]);
  useEffect(() => { if (!cargandoInicial) guardarStorage(CLAVES_STORAGE.historicoIBR, historicoIBR); }, [historicoIBR, cargandoInicial]);
  useEffect(() => { if (!cargandoInicial) guardarStorage(CLAVES_STORAGE.configuracion, configuracion); }, [configuracion, cargandoInicial]);
  useEffect(() => { if (!cargandoInicial) guardarStorage(CLAVES_STORAGE.auditoria, auditoria); }, [auditoria, cargandoInicial]);
  ...
  const registrarAuditoria = (accion, descripcion) => {
    setAuditoria((prev) => [{ id: nuevoId(), fechaHora: new Date().toISOString(), accion, usuario: "Sistema (sin autenticación de usuarios todavía)", descripcion }, ...prev].slice(0, 500));
  };
  ...
  // Navegación desde el Dashboard: abrir un crédito específico en Créditos
  const irACreditoDetalle = (creditoId) => {
    setCreditoAAbrirPendiente(creditoId);
    setModuloInterno("creditos");
  };
  // Navegación desde el Dashboard: abrir Flujo de Caja en un mes específico
  ...
  const api = {
    ...
    crearCredito(data) {
      ...
      setCreditos((prev) => [...prev, { ...data, id: nuevoId() }]);
    ...
    actualizarCredito(id, data) {
      ...
      const validacion = MotorFinanciero.validarCredito(data, historicoIBR);
      setCreditos((prev) => prev.map((c) => (c.id === id ? { ...c, ...data } : c)));
    ...
    agregarTasaIBR(data) {
      setHistoricoIBR((prev) => [...prev, { ...data, id: nuevoId() }].sort((a, b) => a.fecha.localeCompare(b.fecha)));
    ...
    importarCreditosLote(datos) {
      setCreditos((prev) => [...prev, ...datos.map((d) => ({ ...d, id: nuevoId() }))]);
    ...
    importarTasasIBRLote(datos) {
      setHistoricoIBR((prev) => {
        ...
        datos.forEach((d) => {
          ...
          else mapa.set(d.fecha, { ...d, id: nuevoId() });
        });
    ...
    crearBanco(nombre) {
      ...
      setConfiguracion((prev) => ({ ...prev, bancos: [...prev.bancos, { id: nuevoId(), nombre: limpio, activo: true }] }));
    ...
    editarBanco(id, nombre) {
      const limpio = nombre.trim();
      if (!limpio) return;
      setConfiguracion((prev) => ({ ...prev, bancos: prev.bancos.map((b) => (b.id === id ? { ...b, nombre: limpio } : b)) }));
    ...
    alternarActivoBanco(id) {
      setConfiguracion((prev) => ({ ...prev, bancos: prev.bancos.map((b) => (b.id === id ? { ...b, activo: !b.activo } : b)) }));
    ...
    eliminarBanco(id) {
      ...
      setConfiguracion((prev) => ({ ...prev, bancos: prev.bancos.filter((b) => b.id !== id) }));
    ...
    establecerDecimales(n) {
      setConfiguracion((prev) => ({ ...prev, decimales: n }));
    ...
    restaurarDesdeRespaldo(datos) {
      ...
      setHistoricoIBR(datos.historicoIBR || []);
      setConfiguracion(datos.configuracion || CONFIGURACION_INICIAL);
      registrarAuditoria("Restauración de respaldo", `Se restauró el sistema desde un archivo de respaldo (${(datos.creditos || []).length} créditos, ${(datos.historicoIBR || []).length} tasas IBR).`);
